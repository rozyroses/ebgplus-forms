...
const renderAuth = () => {
  app.innerHTML = chrome(`<section class="auth-card"><p class="eyebrow">EBG ACCOUNT</p><h1>Sign in before you apply.</h1><p>Signing in links your submission to EBG+ so you can receive application updates and message the team.</p><form id="login"><label>Email<input name="email" type="email" required></label><label>Password<input name="password" type="password" required></label><button>Sign in</button><p id="status"></p></form><button id="cancel-login" class="ghost">Continue without signing in</button></section>`);
...
const renderForm = async (slug) => {
  ...
  } catch(err) { app.innerHTML=chrome(`<div class="empty"><h1>Something went wrong.</h1><p>${esc(err.message)}</p></div>`); wireAuth(); }
};
